package no.mhl.twemojiiconsandroid.food

import no.mhl.twemojiiconsandroid.core.model.Category
import no.mhl.twemojiiconsandroid.core.model.EmojiIcon
import no.mhl.twemojiiconsandroid.core.model.Font
import no.mhl.twemojiiconsandroid.core.model.Subcategory

object FoodAndDrink {

    // region Food fruit
    private val foodFruit: List<EmojiIcon> = listOf(
        EmojiIcon("Grapes", "1f347", Category.FoodAndDrink, Subcategory.FoodFruit, Font.Twemoji, R.drawable.ic_1f347),
        EmojiIcon("Melon", "1f348", Category.FoodAndDrink, Subcategory.FoodFruit, Font.Twemoji, R.drawable.ic_1f348),
        EmojiIcon("Watermelon", "1f349", Category.FoodAndDrink, Subcategory.FoodFruit, Font.Twemoji, R.drawable.ic_1f349),
        EmojiIcon("Tangerine", "1f34a", Category.FoodAndDrink, Subcategory.FoodFruit, Font.Twemoji, R.drawable.ic_1f34a),
        EmojiIcon("Lemon", "1f34b", Category.FoodAndDrink, Subcategory.FoodFruit, Font.Twemoji, R.drawable.ic_1f34b),
        EmojiIcon("Banana", "1f34c", Category.FoodAndDrink, Subcategory.FoodFruit, Font.Twemoji, R.drawable.ic_1f34c),
        EmojiIcon("Pineapple", "1f34d", Category.FoodAndDrink, Subcategory.FoodFruit, Font.Twemoji, R.drawable.ic_1f34d),
        EmojiIcon("Mango", "1f96d", Category.FoodAndDrink, Subcategory.FoodFruit, Font.Twemoji, R.drawable.ic_1f96d),
        EmojiIcon("RedApple", "1f34e", Category.FoodAndDrink, Subcategory.FoodFruit, Font.Twemoji, R.drawable.ic_1f34e),
        EmojiIcon("GreenApple", "1f34f", Category.FoodAndDrink, Subcategory.FoodFruit, Font.Twemoji, R.drawable.ic_1f34f),
        EmojiIcon("Pear", "1f350", Category.FoodAndDrink, Subcategory.FoodFruit, Font.Twemoji, R.drawable.ic_1f350),
        EmojiIcon("Peach", "1f351", Category.FoodAndDrink, Subcategory.FoodFruit, Font.Twemoji, R.drawable.ic_1f351),
        EmojiIcon("Cherries", "1f352", Category.FoodAndDrink, Subcategory.FoodFruit, Font.Twemoji, R.drawable.ic_1f352),
        EmojiIcon("Strawberry", "1f353", Category.FoodAndDrink, Subcategory.FoodFruit, Font.Twemoji, R.drawable.ic_1f353),
        EmojiIcon("Blueberries", "1fad0", Category.FoodAndDrink, Subcategory.FoodFruit, Font.Twemoji, R.drawable.ic_1fad0),
        EmojiIcon("KiwiFruit", "1f95d", Category.FoodAndDrink, Subcategory.FoodFruit, Font.Twemoji, R.drawable.ic_1f95d),
        EmojiIcon("Tomato", "1f345", Category.FoodAndDrink, Subcategory.FoodFruit, Font.Twemoji, R.drawable.ic_1f345),
        EmojiIcon("Olive", "1fad2", Category.FoodAndDrink, Subcategory.FoodFruit, Font.Twemoji, R.drawable.ic_1fad2),
        EmojiIcon("Coconut", "1f965", Category.FoodAndDrink, Subcategory.FoodFruit, Font.Twemoji, R.drawable.ic_1f965)
    )
    // endregion

    // region Food vegetable
    private val foodVegetable: List<EmojiIcon> = listOf(
        EmojiIcon("Avocado", "1f951", Category.FoodAndDrink, Subcategory.FoodVegetable, Font.Twemoji, R.drawable.ic_1f951),
        EmojiIcon("Eggplant", "1f346", Category.FoodAndDrink, Subcategory.FoodVegetable, Font.Twemoji, R.drawable.ic_1f346),
        EmojiIcon("Potato", "1f954", Category.FoodAndDrink, Subcategory.FoodVegetable, Font.Twemoji, R.drawable.ic_1f954),
        EmojiIcon("Carrot", "1f955", Category.FoodAndDrink, Subcategory.FoodVegetable, Font.Twemoji, R.drawable.ic_1f955),
        EmojiIcon("EarOfCorn", "1f33d", Category.FoodAndDrink, Subcategory.FoodVegetable, Font.Twemoji, R.drawable.ic_1f33d),
        EmojiIcon("HotPepper", "1f336", Category.FoodAndDrink, Subcategory.FoodVegetable, Font.Twemoji, R.drawable.ic_1f336),
        EmojiIcon("BellPepper", "1fad1", Category.FoodAndDrink, Subcategory.FoodVegetable, Font.Twemoji, R.drawable.ic_1fad1),
        EmojiIcon("Cucumber", "1f952", Category.FoodAndDrink, Subcategory.FoodVegetable, Font.Twemoji, R.drawable.ic_1f952),
        EmojiIcon("LeafyGreen", "1f96c", Category.FoodAndDrink, Subcategory.FoodVegetable, Font.Twemoji, R.drawable.ic_1f96c),
        EmojiIcon("Broccoli", "1f966", Category.FoodAndDrink, Subcategory.FoodVegetable, Font.Twemoji, R.drawable.ic_1f966),
        EmojiIcon("Garlic", "1f9c4", Category.FoodAndDrink, Subcategory.FoodVegetable, Font.Twemoji, R.drawable.ic_1f9c4),
        EmojiIcon("Onion", "1f9c5", Category.FoodAndDrink, Subcategory.FoodVegetable, Font.Twemoji, R.drawable.ic_1f9c5),
        EmojiIcon("Mushroom", "1f344", Category.FoodAndDrink, Subcategory.FoodVegetable, Font.Twemoji, R.drawable.ic_1f344),
        EmojiIcon("Peanuts", "1f95c", Category.FoodAndDrink, Subcategory.FoodVegetable, Font.Twemoji, R.drawable.ic_1f95c),
        EmojiIcon("Chestnut", "1f330", Category.FoodAndDrink, Subcategory.FoodVegetable, Font.Twemoji, R.drawable.ic_1f330)
    )
    // endregion

    // region Food prepared
    private val foodPrepared: List<EmojiIcon> = listOf(
        EmojiIcon("Bread", "1f35e", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f35e),
        EmojiIcon("Croissant", "1f950", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f950),
        EmojiIcon("BaguetteBread", "1f956", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f956),
        EmojiIcon("Flatbread", "1fad3", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1fad3),
        EmojiIcon("Pretzel", "1f968", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f968),
        EmojiIcon("Bagel", "1f96f", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f96f),
        EmojiIcon("Pancakes", "1f95e", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f95e),
        EmojiIcon("Waffle", "1f9c7", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f9c7),
        EmojiIcon("CheeseWedge", "1f9c0", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f9c0),
        EmojiIcon("MeatOnBone", "1f356", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f356),
        EmojiIcon("PoultryLeg", "1f357", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f357),
        EmojiIcon("CutOfMeat", "1f969", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f969),
        EmojiIcon("Bacon", "1f953", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f953),
        EmojiIcon("Hamburger", "1f354", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f354),
        EmojiIcon("FrenchFries", "1f35f", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f35f),
        EmojiIcon("Pizza", "1f355", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f355),
        EmojiIcon("HotDog", "1f32d", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f32d),
        EmojiIcon("Sandwich", "1f96a", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f96a),
        EmojiIcon("Taco", "1f32e", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f32e),
        EmojiIcon("Burrito", "1f32f", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f32f),
        EmojiIcon("Tamale", "1fad4", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1fad4),
        EmojiIcon("StuffedFlatbread", "1f959", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f959),
        EmojiIcon("Falafel", "1f9c6", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f9c6),
        EmojiIcon("Egg", "1f95a", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f95a),
        EmojiIcon("Cooking", "1f373", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f373),
        EmojiIcon("ShallowPanOfFood", "1f958", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f958),
        EmojiIcon("PotOfFood", "1f372", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f372),
        EmojiIcon("Fondue", "1fad5", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1fad5),
        EmojiIcon("BowlWithSpoon", "1f963", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f963),
        EmojiIcon("GreenSalad", "1f957", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f957),
        EmojiIcon("Popcorn", "1f37f", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f37f),
        EmojiIcon("Butter", "1f9c8", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f9c8),
        EmojiIcon("Salt", "1f9c2", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f9c2),
        EmojiIcon("CannedFood", "1f96b", Category.FoodAndDrink, Subcategory.FoodPrepared, Font.Twemoji, R.drawable.ic_1f96b)
    )
    // endregion

    // region Food asian
    private val foodAsian: List<EmojiIcon> = listOf(
        EmojiIcon("BentoBox", "1f371", Category.FoodAndDrink, Subcategory.FoodAsian, Font.Twemoji, R.drawable.ic_1f371),
        EmojiIcon("RiceCracker", "1f358", Category.FoodAndDrink, Subcategory.FoodAsian, Font.Twemoji, R.drawable.ic_1f358),
        EmojiIcon("RiceBall", "1f359", Category.FoodAndDrink, Subcategory.FoodAsian, Font.Twemoji, R.drawable.ic_1f359),
        EmojiIcon("CookedRice", "1f35a", Category.FoodAndDrink, Subcategory.FoodAsian, Font.Twemoji, R.drawable.ic_1f35a),
        EmojiIcon("CurryRice", "1f35b", Category.FoodAndDrink, Subcategory.FoodAsian, Font.Twemoji, R.drawable.ic_1f35b),
        EmojiIcon("SteamingBowl", "1f35c", Category.FoodAndDrink, Subcategory.FoodAsian, Font.Twemoji, R.drawable.ic_1f35c),
        EmojiIcon("Spaghetti", "1f35d", Category.FoodAndDrink, Subcategory.FoodAsian, Font.Twemoji, R.drawable.ic_1f35d),
        EmojiIcon("RoastedSweetPotato", "1f360", Category.FoodAndDrink, Subcategory.FoodAsian, Font.Twemoji, R.drawable.ic_1f360),
        EmojiIcon("Oden", "1f362", Category.FoodAndDrink, Subcategory.FoodAsian, Font.Twemoji, R.drawable.ic_1f362),
        EmojiIcon("Sushi", "1f363", Category.FoodAndDrink, Subcategory.FoodAsian, Font.Twemoji, R.drawable.ic_1f363),
        EmojiIcon("FriedShrimp", "1f364", Category.FoodAndDrink, Subcategory.FoodAsian, Font.Twemoji, R.drawable.ic_1f364),
        EmojiIcon("FishCakeWithSwirl", "1f365", Category.FoodAndDrink, Subcategory.FoodAsian, Font.Twemoji, R.drawable.ic_1f365),
        EmojiIcon("MoonCake", "1f96e", Category.FoodAndDrink, Subcategory.FoodAsian, Font.Twemoji, R.drawable.ic_1f96e),
        EmojiIcon("Dango", "1f361", Category.FoodAndDrink, Subcategory.FoodAsian, Font.Twemoji, R.drawable.ic_1f361),
        EmojiIcon("Dumpling", "1f95f", Category.FoodAndDrink, Subcategory.FoodAsian, Font.Twemoji, R.drawable.ic_1f95f),
        EmojiIcon("FortuneCookie", "1f960", Category.FoodAndDrink, Subcategory.FoodAsian, Font.Twemoji, R.drawable.ic_1f960),
        EmojiIcon("TakeoutBox", "1f961", Category.FoodAndDrink, Subcategory.FoodAsian, Font.Twemoji, R.drawable.ic_1f961)
    )
    // endregion

    // region Food marine
    private val foodMarine: List<EmojiIcon> = listOf(
        EmojiIcon("Crab", "1f980", Category.FoodAndDrink, Subcategory.FoodMarine, Font.Twemoji, R.drawable.ic_1f980),
        EmojiIcon("Lobster", "1f99e", Category.FoodAndDrink, Subcategory.FoodMarine, Font.Twemoji, R.drawable.ic_1f99e),
        EmojiIcon("Shrimp", "1f990", Category.FoodAndDrink, Subcategory.FoodMarine, Font.Twemoji, R.drawable.ic_1f990),
        EmojiIcon("Squid", "1f991", Category.FoodAndDrink, Subcategory.FoodMarine, Font.Twemoji, R.drawable.ic_1f991),
        EmojiIcon("Oyster", "1f9aa", Category.FoodAndDrink, Subcategory.FoodMarine, Font.Twemoji, R.drawable.ic_1f9aa)
    )
    // endregion

    // region All
    val all: List<EmojiIcon> = listOf(
        foodFruit,
        foodVegetable,
        foodPrepared,
        foodAsian,
        foodMarine
    ).flatten()
    // endregion

}